...
export default {
  ...
  setup() {
    ...
    const slideWidth = ref(600); // 1つのスライドの幅（300pxに調整）
    const gap = 20; // スライド間の余白
    const slideWidthWithGap = ref(slideWidth.value + gap); // 幅 + 余白
    const visibleSlides = 6; // 表示するスライド数を6に設定
    ...
    const prevSlide = () => {
      currentSlide.value =
        (currentSlide.value - 1 + requests.length) % requests.length;
      updateSlideStyle();
    };

    const nextSlide = () => {
      currentSlide.value = (currentSlide.value + 1) % requests.length;
      updateSlideStyle();
    };
